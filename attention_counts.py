...
def attention_weighted_counts(newspaper='Guardian', reference_word='girl', model_path='bert-base-uncased', save_name=''):
    '''
    ...
    Example Usage:
    attention_weighted_counts('Breitbart', 'girl', 'bert-base-uncased', 11, 0, breitbart_girl_bertbase)
    '''
    ...
    #load emotion words
    emotionwords = pd.read_excel('./input_dir_wdcts/NRC-Emotion-Lexicon-v0.92-In105Languages-Nov2017Translations.xlsx', usecols="A,DB:DK")
    ...
    news = load_dataset('csv', script_version='master', data_files=['./input_dir_wdcts/articles1.csv', './input_dir_wdcts/articles2.csv', './input_dir_wdcts/articles3.csv'],
    ...
    #w_cts = [[(idx.item(), attention.item()/N) for idx, attention in zip(input_ids[sentence_id], attentions[layer_id][sentence_id, attention_head_id, input_ids[sentence_id].tolist().index(idx),:])] for sentenc$
    ...
    a_file = open('./test_output/'+save_name+'weighted_counts.json', "w")
